...
def create_search_templates(pos_tags, selections, n_gram_max):
    ...
    for i in(range(free_spaces + 1)):
        ...
        for k in range(i):
            ...
            new_pos_tags.append(np.zeros(pos_tags.shape[1], dtype=pos_tags.dtype))
            new_selections.append(np.zeros(selections.shape[1], dtype=selections.dtype))
...
def find_semantic_pairs(n_max = -1,
    ...
    # Load rule file
    semantic_pairs_file = os.path.join(rule_file_directory, semantic_pairs_file)
    ...
    with open(semantic_pairs_file, 'r') as f:
        ...
        for rule_text in csv_reader:
            ...
            print("\nFinding Semantic Pair Type %2d: %s " % (iterations, base_pair))
            print(configx.BREAK_LINE)
